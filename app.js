...
prayerForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await addDoc(collection(db, "prayer_private_contacts"), {
    ...
    console.error(error);
    showNotice("Something went wrong while submitting your prayer request.", "error");
  } finally {
...
async function loadPrayerRequests() {
  try {
    const prayerQuery = query(collection(db, "prayer_requests"), where("status", "==", "approved"), orderBy("createdAt", "desc"));
    ...
    console.error(error);
    prayerGrid.innerHTML = `<div class="empty">Failed to load prayer requests. Firestore may need an index for status and createdAt.</div>`;
  }
}

function renderPrayerRequests(requests) {
  if (!requests.length) {
    prayerGrid.innerHTML = `<div class="empty">No approved prayer requests are currently posted.</div>`;
    return;
  }
    ...
    return `
      <article class="prayer-card">
        ...
        <div class="card-footer">
          <div class="meta"><span>${createdDate}</span><span>❤️ ${request.prayerCount || 0} prayed</span></div>
          <button class="btn btn-primary btn-block pray-button" data-id="${request.id}">Pray For This Request</button>
          <button class="btn btn-secondary btn-block report-button" data-id="${request.id}">Report Request</button>
        </div>
      </article>`;
  }).join("");
  ...
  document.querySelectorAll(".pray-button").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        const contactSnapshot = await getDocs(query(collection(db, "prayer_private_contacts"), where("requestId", "==", requestId)));
        let email = "";
        contactSnapshot.forEach((contactDoc) => { email = contactDoc.data().email || ""; });
...
function attachReportButtons() {
  document.querySelectorAll(".report-button").forEach((button) => {
    button.addEventListener("click", async () => {
      const requestId = button.dataset.id;
      const reason = prompt("Why are you reporting this request? Examples: spam, inappropriate content, personal information, harassment, or safety concern.");
      if (!reason || !reason.trim()) return;

      try {
        button.disabled = true;
        button.textContent = "Reporting...";
        await addDoc(collection(db, "reports"), { requestId, reason: reason.trim(), createdAt: serverTimestamp(), status: "open" });
        await updateDoc(doc(db, "prayer_requests", requestId), { reportCount: increment(1), updatedAt: serverTimestamp() });
        button.textContent = "✓ Reported";
      } catch (error) {
        console.error(error);
        button.textContent = "Report Failed";
        setTimeout(() => { button.textContent = "Report Request"; button.disabled = false; }, 2200);
      }
    });
  });
}
...
function filterRequests() {
  ...
  const filteredRequests = allRequests.filter((request) => {
    const matchesSearch = request.title.toLowerCase().includes(searchValue) || request.message.toLowerCase().includes(searchValue);
...
function updateStatistics() {
  ...
  totalPrayersElement.textContent = allRequests.reduce((sum, request) => sum + (request.prayerCount || 0), 0);
  urgentRequestsElement.textContent = allRequests.filter((request) => request.urgent).length;
}

function showNotice(message, type = "success") {
  formNotice.textContent = message;
  formNotice.className = `notice show ${type}`;
  setTimeout(() => { formNotice.className = "notice"; }, 7000);
}
